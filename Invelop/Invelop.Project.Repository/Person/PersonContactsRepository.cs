...
namespace Invelop.Project.Repository.Person
{
    public class PersonContactsRepository : IPersonContactsRepository
    {
        ...
        public async Task<long> Insert(PersonContacts personContacts)
        {
            ...
            long? dateOfBirthEpoch = GetEpochDate(personContacts.DateOfBirth);
        ...
        private static long? GetEpochDate(DateTime? date)
        {
            return date.HasValue ? new DateTimeOffset(date.Value.Date, new TimeSpan()).ToUnixTimeSeconds() : null;
        }
    }
}
